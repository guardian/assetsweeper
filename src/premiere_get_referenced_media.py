...
class ProcessThread(threading.Thread):
    def __init__(self,queue,out_queue,cfg,*args,**kwargs):
        super(ProcessThread,self).__init__(*args,**kwargs)
        self.queue = queue
        self.out_queue = out_queue
        self.cfg = cfg
        logging.info("Connecting to index on %s" % cfg.value('elasticsearch', noraise=True))
    ...
    def run(self):
        ...
        while True:
            ...
            if filepath is None:
                logging.info("Received null filepath, so terminating thread")
                break
            try:
                #result_tuple is in the form of (project_refs, no_vsitem, not_in_db)
                result_tuple = self.process_premiere_project(filepath,self.db,self.cfg)
                self.out_queue.put(result_tuple)
                lg.info(
                    ...
                    (filepath, result_tuple[0], result_tuple[1], result_tuple[2]))
            except NoMediaError:
...
parser.add_option("-n", "--not-incremental", dest="fullrun", action="store_true",
...
if lastruntime is None:
    ...
    if options.force:
        ...
        warnings.append("--force has been specified so running anyway")
        break_lock = True
    ...
    if laststarttime is None:
        ...
        warnings.append("No start time could be found for existing run. Over-riding lock")
        break_lock = True
    ...
    if lag.days > 0:
        ...
        warnings.append("Last start is more than 1 day ago, so assuming lock is stale.")
        break_lock = True
...
for w in warnings:
    #log warnings now we are registered
    db.insert_sysparam("warning",w)

input_queue = Queue.Queue()
output_queue = Queue.Queue()
threadlist = []

logging.info("Starting up {0} threads".format(THREADS))
for n in range(1,THREADS):
    t = ProcessThread(input_queue,output_queue,cfg)
    t.start()
    threadlist.append(t)
...
try:
    ...
    #get the results off the output queue
    try:
        while True:
            (project_refs, no_vsitem, not_in_db) = output_queue.get_nowait()
